  ...
  if ~isempty(uid_Parcel)
    for j=1:length(uid_Parcel)
      ...
      for k=1:length(uid_Vertices)
        ...
        Parcel(j).Vertices{k} = str2num(get(vertices, children(vertices, find(vertices, 'Vertices')), 'value'));
      end
    end
  end
...
for i=1:length(Cifti.MatrixIndicesMap)
  switch Cifti.MatrixIndicesMap(i).IndicesMapToDataType
    ...
    case 'CIFTI_INDEX_TYPE_PARCELS' 
      ...
      % FIXME make use of Cifti.Surface
      
      dimord(Cifti.MatrixIndicesMap(i).AppliesToMatrixDimension+1) = {'chan'};
      source.label = {Cifti.Parcel(:).Name};
      
      tmp1 = {};
      tmp2 = [];
      for j=1:numel(Cifti.Parcel)
        tmp1{end+1} = Cifti.Parcel(j).Name;
        tmp2(end+1) = sum(cellfun(@numel, Cifti.Parcel(j).Vertices));
      end
      ...
      tmp2 = [];
      for j=1:numel(Cifti.Parcel)
        for k=1:numel(Cifti.Parcel(j).BrainStructure)
          tmp1{end+1} = Cifti.Parcel(j).BrainStructure{k};
          tmp2(end+1) = numel(Cifti.Parcel(j).Vertices{k});
        end
      end
      begpos = [0 cumsum(tmp2(1:end-1))] + 1;
      endpos = begpos + tmp2 - 1;
      ...
      for j=1:max(Parcellation)
        for k=1:max(BrainStructure)
          ...
          Cifti.greynodeIndex{end+1} = Cifti.Parcel(j).Vertices{k} + 1; % convert from zero to one-offset
        end
      end
...
if hasbrainmodel
  ...
  elseif any(strcmp(dimord, 'chan'))
    ...
    source.brainordinate = brainordinate;
    % from now on treat it like parcels
    hasbrainmodel = false;
  end
end % if hasbrainmodel
